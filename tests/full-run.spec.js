...
test('complete a real freeride run, persist a medal, unlock a board, and ride again', async ({ page }) => {
  ...
  const progress = await page.evaluate(() => JSON.parse(localStorage.getItem('wntr-ski:progress:v1')));
  ...
  await expect(page.locator('#result-text')).toContainText(record.score.toLocaleString());
  await page.screenshot({ path: 'test-results/full-run-result.png' });

  await page.locator('#home').click();
  await expect(page.locator('#lodge')).toBeVisible();
  await expect(page.locator('[data-course="north"] .record')).toContainText(`BEST ${record.score}`);
  ...
  await expect(page.locator('#score')).not.toHaveText('00000', { timeout: 10000 });
  await page.keyboard.up('KeyX');
